        ...
        <StackLayout
          ...
          <FlexboxLayout class="message-bubble-content">
            <FlexboxLayout class="meta">
              <Image :src="`https://kotchan.org/icons/countries2/${message.country}.png`" width="22" height="22"></Image>
              <Label
                      :text="message.date"
                      class="time"></Label>
              <!--              <Label-->
              ...
              <!--                      :class.blue="isViewed(message)"></Label>-->
            </FlexboxLayout>
            <Image v-if="message.thumb" :src="message.thumb.replace('/home/ph/livechan-js/public', 'https://kotchan.org')" stretch="fill"></Image>
            <Label class="message-text"
                   textWrap="true">{{message.body}}</Label>
          </FlexboxLayout>
        </StackLayout>
      </FlexboxLayout>
    </ScrollView>
    ...
    <FlexboxLayout class="message-box">
      ...
      cornerRadius: 50
        ...
        <TextView
                ...
                @submit="post()"
        ></TextView>
          <Button @click="post">miu</Button>
          <Label class="mdi-attach-file"  />
        <Label class="mdi-photo-camera"  />
      </FlexboxLayout>
    </FlexboxLayout>
  </StackLayout>
</GridLayout>
...
<script >
    ...
    // import Label from 'nativescript-vue-Label'
  ...
  export default {
    data() {
      ...
      mounted() {
        ...
        const socket = new TNSPusher('6fdb5caf8dfd70d25132', {
          cluster: 'eu',encrypted: true
        });

        const channel = socket.subscribe('int');
      ...
      methods: {
        fetch: () => {
        httpModule.getJSON("https://kotchan.org/last/int", {last_count:this.last_count}).then((r) => {
            this.posts = r;
            this.last_count=r[r.length-1].count;
        }, (e) => {
        });

    },
        post: () => {
          axios.post("https://kotchan.org/chat/int", this.form)
          .then((res) => {
